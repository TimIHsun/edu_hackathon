...
summercamp <- l %>%
  ...
  summarise(
    ...
    reg_date = grepl("^截止：",short_descrip) %>% {ifelse(sum(.) == 0, NA,short_descrip[.])},
    reg_date = ifelse(is.na(reg_date),NA, 
                      str_remove(short_descrip[grep("^截止：",short_descrip)],"[\u4E00-\u9FFF]+(?=：)：")),
    ...
    age_limit = short_descrip %>% .[str_which(.,"^對象：")] %>% 
      str_remove(.,"[\u4E00-\u9FFF]+(?=：)：") %>% paste0(.,collapse = ";"),
    description = short_descrip[NROW(title)],
    .groups = "drop"
  ) %>%
